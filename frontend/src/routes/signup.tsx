  ...
  return (
    <div className="font-body text-on-surface bg-surface min-h-screen flex">
      <main className="flex-1 flex items-center justify-center p-8 order-2 lg:order-1 relative">
        <Link to="/" className="absolute top-6 left-6 text-xs font-bold text-on-surface-variant hover:text-primary inline-flex items-center gap-1">
        ...
        <div className="w-full max-w-md">
          <div className="bg-surface-container-lowest p-10 rounded-2xl shadow-[0px_20px_40px_rgba(25,28,30,0.04)] border border-outline-variant/15">
            <div className="mb-8">
              ...
              <p className="text-on-surface-variant text-sm mt-1">Spin up a fraud command center in under a minute.</p>
            </div>
            <form className="space-y-5" onSubmit={onSubmit}>
              <Field label="Full Name" icon="person">
                <input value={name} onChange={(e) => setName(e.target.value)} className={inputCls} placeholder="Priya Kapur" />
              </Field>
              <Field label="Organization" icon="domain">
                <input value={org} onChange={(e) => setOrg(e.target.value)} className={inputCls} placeholder="Acme Payments Pvt Ltd" />
              </Field>
              <Field label="Corporate Email" icon="alternate_email">
                <input value={email} onChange={(e) => setEmail(e.target.value)} type="email" required className={inputCls} placeholder="name@organization.com" />
              </Field>
              <Field label="Access Key" icon="lock">
                <input value={password} onChange={(e) => setPassword(e.target.value)} type="password" required className={inputCls} placeholder="At least 6 characters" />
              </Field>
              <label className="flex items-start gap-3 p-3 bg-surface-container rounded-lg cursor-pointer">
                <input type="checkbox" checked={agree} onChange={(e) => setAgree(e.target.checked)} className="w-4 h-4 mt-0.5 rounded-sm" />
                <span className="text-xs text-on-surface-variant">I agree to the Corporate Policy and Service Terms.</span>
              </label>
              {err && <p className="text-error text-sm font-semibold">{err}</p>}
              <button disabled={busy} type="submit" className="signature-gradient w-full text-white font-headline font-bold py-3.5 rounded-xl shadow-lg active:scale-[0.98] transition-all disabled:opacity-60 flex items-center justify-center gap-2">
      ...
      <aside className="hidden lg:flex w-1/2 relative overflow-hidden signature-gradient text-white p-12 flex-col justify-between order-1 lg:order-2">
        ...
        <div className="relative z-10 max-w-md">
          ...
          <p className="text-white/80 text-sm leading-relaxed">
            Tier-aware risk scoring, live geo-anomaly detection, and one-click block actions across India's payment rails.
          </p>
...
function Field({ label, icon, children }: { label: string; icon: string; children: React.ReactNode }) {
  return (
    <div className="space-y-2">
      <label className="font-label text-xs font-semibold text-on-surface-variant uppercase tracking-wider">{label}</label>
